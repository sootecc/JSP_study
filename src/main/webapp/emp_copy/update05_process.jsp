...
<%
	...
	//폼에서 넘겨 받은 eno와 passwd를 DB에서 가져온 eno, PASSWD와 비교하여 같으면 update
	
	try{
		//폼에서 넘겨 받은 eno를 조건으로 해서 DB의 값을 select 해온다.
		sql = String.format("SELECT eno, pass FROM emp_copy WHERE eno =? ");
		pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, eno);		//index는 1번부터
		...
		if(rs.next()){	// DB에 폼에서 넘긴 eno가 존재하면 -> 폼에서 넘긴 패스워드와 DB의 Password가 일치하는지 확인
			...
			String rPassword =rs.getString("ename");		//DB 컬럼명
			
			//폼에서 넘겨준 값과 DB에서 가져온 값이 일치하는지 확인
			if(eno.equals(reno) && ename.equals(rPassword)){
				//DB에서 가져온 pass와 폼에서 넘긴 passwd가 일치할 때 update
				sql = String.format("UPDATE mbTbl set eno = ?, ename = ? WHERE eno = ?");
