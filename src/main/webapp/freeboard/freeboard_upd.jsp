...
<SCRIPT language="javascript">
	function check() {
		with (document.msgwrite) {		//Document 객체는 웹 페이지 그 자체를 의미합니다. 웹 페이지에 존재하는 HTML 요소에 접근하고자 할 때는 반드시 Document 객체부터 시작해야 합니다.
...
<BODY>
	...
	<P>
		...
		ResultSet rs = null;
		int cnt = 0;
		int id = Integer.parseInt(request.getParameter("id"));		//  request.getParameter는 모두 String값으로 들어오기 때문에 parseInt와 같은 조치가 필요하다.
		...
		%>
			...
			<FORM name="msgwrite" method=POST action="freeboard_upddb.jsp?id=<%=id%>&page=<%=p%>">		<!--  form에서 submit을 호출하면 form 안에 있는 모든 변수값을 action으로 던져준다. -->
